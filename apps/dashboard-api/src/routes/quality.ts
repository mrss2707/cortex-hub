...
sessionsRouter.post('/start', async (c) => {
  try {
    const body = await c.req.json()
    const { action, project, agentId, hostname, os, ide, branch, capabilities, role } = body
    const sessionId = `sess_${Date.now()}`

    const stmt = db.prepare(
      `INSERT INTO session_handoffs (id, from_agent, project, task_summary, context, hostname, os, ide, branch, capabilities, role, last_activity)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))`
    )
    stmt.run(
      sessionId,
